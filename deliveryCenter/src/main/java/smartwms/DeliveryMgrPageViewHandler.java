...
@Service
public class DeliveryMgrPageViewHandler {


    @Autowired
    private DeliveryMgrPageRepository deliveryMgrPageRepository;
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryOrderChanged_then_UPDATE_4(@Payload DeliveryOrderChanged deliveryOrderChanged) {
        try {
            if (deliveryOrderChanged.isMe()) {
                // view 객체 조회
                List<> List = Repository.findByDeliveryOrderId(.getDeliveryOrderId());
                for(  : List){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
